...
When tests run against a Postgres testcontainer (typically via
``pytest-testcontainers-django``), the baseline is loaded **inside**
the container at startup (via Postgres'
``/docker-entrypoint-initdb.d/`` mechanism) and the test DB is
created with ``CREATE DATABASE … WITH TEMPLATE``, so this module's
host ``psql`` path is **not** invoked. It still runs for the
``baseline_load`` management command and for any external mechanism
that pre-populates the test DB without touching ``TEST.TEMPLATE``,
where a host ``psql`` is expected to be available.
"""
...
def load_baseline(
    ...
    ``HOST`` and ``PORT`` are passed through verbatim — empty ``HOST``
    is preserved (Django's unix-socket case), not rewritten to
    ``localhost``.
    """
