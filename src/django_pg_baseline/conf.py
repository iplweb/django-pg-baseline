...
class BaselineConfig:
    """Typed view of ``settings.PG_BASELINE``.

    Frozen so call sites can pass it around without worrying about
    mutation. CLI overrides should construct a new instance via
    ``dataclasses.replace(cfg, rebuild_image=...)`` rather than mutating
    in place — this avoids time-of-check / time-of-use bugs when
    multiple call sites read the same config.
    """
    ...
    pg_dump_extra_args: tuple[str, ...] = field(
        default_factory=lambda: tuple(DEFAULT_PG_DUMP_EXTRA_ARGS)
    )
    freeze_timestamps: tuple[tuple[str, tuple[str, ...]], ...] = field(
        default_factory=lambda: tuple(
            (t, tuple(cols)) for (t, cols) in DEFAULT_FREEZE_TIMESTAMPS
        )
    )
...
def get_config() -> BaselineConfig:
    """Assemble a :class:`BaselineConfig` from ``settings.PG_BASELINE``.
    ...
    ``PG_BASELINE`` is unset, empty, or missing ``BASELINE_DIR``.
    """
    ...
    return BaselineConfig(
        ...
        pg_dump_extra_args=tuple(pg_dump_extra_args),
        freeze_timestamps=tuple(freeze_timestamps_list),
